    ...
    <AuthenticatedLayout user={auth?.user}>
      ...
      <div className="p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-semibold" data-help-key="transactions_index_title">{t('transactions')}</h1>
          <Link href={route('transactions.create')} className="px-3 py-2 bg-indigo-600 text-white rounded">{t('new_transaction')}</Link>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr>
                  <th className="text-left px-4 py-2" data-help-key="transactions_th_date">{t('date')}</th>
                  <th className="text-left px-4 py-2" data-help-key="transactions_th_counterparty">{t('counterparty_type')}</th>
                  <th className="text-left px-4 py-2" data-help-key="transactions_th_direction">{t('direction')}</th>
                  <th className="text-left px-4 py-2" data-help-key="transactions_th_source">{t('source')}</th>
                  <th className="text-right px-4 py-2" data-help-key="transactions_th_amount">{t('amount')}</th>
                </tr>
              </thead>
              <tbody>
                {(loans?.data || []).map(ln => (
