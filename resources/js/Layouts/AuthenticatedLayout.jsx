...
function NavTabs() {
    const tabs = useMemo(() => ([
        ...
        { key: 'products', label: 'Products', href: route('products.index'), active: route().current('products.*') },
        { key: 'customers', label: 'Customers', href: route('customers.index'), active: route().current('customers.*') },
        { key: 'categories', label: 'Categories', href: route().has('categories.index') ? route('categories.index') : '#', active: route().current('categories.*') },
        { key: 'vendors', label: 'Vendors', href: route().has('vendors.index') ? route('vendors.index') : '#', active: route().current('vendors.*') },
        ...
        { key: 'invloans', label: 'Inventory Loans', href: route().has('inventory.loans.index') ? route('inventory.loans.index') : '#', active: route().current('inventory.loans.*') },
        ...
        { key: 'pl', label: 'Profit & Loss', href: route().has('reports.accounting.profitLoss') ? route('reports.accounting.profitLoss') : '#', active: route().current('reports.accounting.profitLoss') },
        { key: 'inventory', label: 'Inventory', href: route().has('inventory.adjustments.index') ? route('inventory.adjustments.index') : '#', active: route().current('inventory.adjustments.*') },
    ]), []);

    return (
        <div className="hidden sm:flex min-w-0 flex-1 items-center">
            <div className="flex items-center gap-6 overflow-x-auto whitespace-nowrap w-full pr-2">
                {tabs.map((t) => (
                    <div key={t.key} className="shrink-0">
                        <NavLink href={t.href} active={t.active}>{t.label}</NavLink>
                    </div>
                ))}
            </div>
        </div>
            ...
            <nav className="border-b border-gray-100 bg-white">
                ...
                >
                    <div className="space-y-1 pb-3 pt-2">
                        ...
                        <ResponsiveNavLink href={route().has('categories.index') ? route('categories.index') : '#'} active={route().current('categories.*')}>
                            Categories
                        </ResponsiveNavLink>
                        <ResponsiveNavLink href={route().has('vendors.index') ? route('vendors.index') : '#'} active={route().current('vendors.*')}>
                            Vendors
                        </ResponsiveNavLink>
                        ...
                        <ResponsiveNavLink href={route().has('pricing.index') ? route('pricing.index') : '#'} active={route().current('pricing.*')}>
                            Pricing Rules
                        </ResponsiveNavLink>
                        ...
                        <ResponsiveNavLink href={route().has('shops.index') ? route('shops.index') : '#'} active={route().current('shops.index')}>
                            Shops
                        </ResponsiveNavLink>
                        <ResponsiveNavLink href={route().has('reports.sales') ? route('reports.sales') : '#'} active={route().current('reports.*')}>
                            Reports
                        </ResponsiveNavLink>
                        <ResponsiveNavLink href={route().has('reports.purchases') ? route('reports.purchases') : '#'} active={route().current('reports.purchases')}>
                            Purchases Report
                        </ResponsiveNavLink>
                        <ResponsiveNavLink href={route().has('inventory.loans.index') ? route('inventory.loans.index') : '#'} active={route().current('inventory.loans.*')}>
                            Inventory Loans
                        </ResponsiveNavLink>
                        <ResponsiveNavLink href={route().has('reports.sales') ? route('reports.sales') : '#'} active={route().current('reports.sales')}>
                            Sales Report
                        </ResponsiveNavLink>
                        ...
                        <ResponsiveNavLink href={route().has('reports.accounting.trialBalance') ? route('reports.accounting.trialBalance') : '#'} active={route().current('reports.accounting.trialBalance')}>
                            Trial Balance
                        </ResponsiveNavLink>
                        <ResponsiveNavLink href={route().has('reports.accounting.profitLoss') ? route('reports.accounting.profitLoss') : '#'} active={route().current('reports.accounting.profitLoss')}>
                            Profit & Loss
                        </ResponsiveNavLink>
                        <ResponsiveNavLink href={route().has('inventory.adjustments.index') ? route('inventory.adjustments.index') : '#'} active={route().current('inventory.adjustments.*')}>
                            Inventory
                        </ResponsiveNavLink>
                    </div>
